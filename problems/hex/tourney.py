...
PLAYERS = [	
	'problems.hex.random_player',
	'problems.hex.rush_player',
			]
...
def tourney(players):
	...
	tot = len(PLAYERS) + len(players)

	log("===============")
	log(f"Tourney Board Size {SIZE}")
	log("===============")

	players = [Player(name, play) for name, play in players]
	points = [0] * tot
	for p in PLAYERS:
		mod = load(p)
